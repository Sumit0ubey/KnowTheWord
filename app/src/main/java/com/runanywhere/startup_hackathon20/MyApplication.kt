...
class MyApplication : Application() {
    ...
    /**
     * Device resource manager for monitoring RAM, battery, and storage.
     */
    val deviceResourceManager: DeviceResourceManager by lazy {
        DeviceResourceManager(this)
    }

    /**
     * Adaptive model manager for intelligent model selection and loading.
     * Auto-selects appropriate model based on device capabilities.
     */
    val adaptiveModelManager: AdaptiveModelManager by lazy {
        AdaptiveModelManager(this)
    }

    /**
     * Performance booster for maximizing LLM generation speed.
     * Activates high-priority mode during inference.
     */
    val performanceBooster: PerformanceBooster by lazy {
        PerformanceBooster(this)
    }

    /**
     * Image analyzer using ML Kit for actual image understanding.
     * Detects objects, text (OCR), and faces in images.
     */
    val imageAnalyzer: ImageAnalyzer by lazy {
        ImageAnalyzer(this)
    }

    // Flag to track if SDK is initialized
    ...
    private suspend fun initializeSDK() {
        try {
            ...
            // Step 1: Log device resources first
            deviceResourceManager.logResourceState()
            ...
            // Step 3: Register LLM Service Provider
            ...
            // Step 6: Initialize Adaptive Model Manager
            // Small delay to ensure models list is ready
            delay(500)
            initializeAdaptiveModelManager()
    ...
    /**
     * Initializes the Adaptive Model Manager and optionally auto-loads appropriate model.
     * This runs after SDK is fully initialized.
     */
    private suspend fun initializeAdaptiveModelManager() {
        try {
            Log.i(TAG, "Initializing Adaptive Model Manager...")

            // Get device tier and log recommendation
            val tier = deviceResourceManager.getResourceSnapshot().deviceTier
            val recommendation = adaptiveModelManager.getModelRecommendation()
            Log.i(TAG, "Device Tier: $tier")
            Log.i(TAG, recommendation)

            // Initialize with auto-load enabled
            // This will download (if needed) and load the most appropriate model
            adaptiveModelManager.initialize(autoLoad = true)

            Log.i(TAG, "Adaptive Model Manager initialized")

        } catch (e: Exception) {
            Log.e(TAG, "Adaptive Model Manager initialization failed: ${e.message}")
        }
    }

    /**
     * Checks if SDK is fully initialized and ready.
     */
    fun isSDKReady(): Boolean = sdkInitialized
